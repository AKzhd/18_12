...
int main() {
  ...
  std::fstream users_states_file = std::fstream("users_states_file.txt", std::ios::in | std::ios::out);
    ...
    fs::permissions("users_states_file.txt",fs::perms::owner_all);           //Назначаем права на файл только для владельца (rwx)
  }
  ...
  fs::permissions("msg_states_file.txt",fs::perms::owner_all);
